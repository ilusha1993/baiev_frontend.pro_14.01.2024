...
let filteredUsers = users.filter(function (user){
    return user.balance.replace('$', '').replace(',', '') > 2000;
});
console.log('Телефонні номери користувачів, у яких баланс більше 2000 доларі: ');
console.log(filteredUsers.map(function (user){
    return user.phone;
}));
console.log('Сума всіх балансів користувачів: ');
console.log(users.reduce(function (sum, user){
    return sum + parseFloat(user.balance.replace('$', '').replace(',', ''));
}, 0));
